...
func binarySearchRecv(array []int, target int, left int, right int) int {
	middle := (left + right) / 2
	if target == array[middle] {
		return middle
	} else if middle < right && target > array[middle] {
		return binarySearchRecv(array, target, middle+1, right)
	} else if middle > left && target < array[middle] {
		return binarySearchRecv(array, target, left, middle-1)
	}
